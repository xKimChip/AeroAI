...
Original file is located at
    https://colab.research.google.com/drive/1GS9R682isJMte4PvEaPDsbktVuBBQW1L
"""
...
def create_synthetic_anomalies(df, num_anomalies=1000):
    ...
    # Define anomaly types with probabilities
    anomaly_types = [
        # Speed anomalies (35% chance)
        {'type': 'high_speed', 'prob': 0.15},
        {'type': 'low_speed_high_alt', 'prob': 0.10},
        {'type': 'sudden_speed_change', 'prob': 0.10},

        # Altitude anomalies (25% chance)
        {'type': 'excessive_altitude', 'prob': 0.10},
        {'type': 'low_altitude_high_speed', 'prob': 0.05},
        {'type': 'rapid_altitude_drop', 'prob': 0.10},
        ...
        # New anomaly type (10% chance)
        {'type': 'disappearing_reappearing', 'prob': 0.10}
    ]
    ...
    for _ in range(num_anomalies):
        ...
        elif anomaly_type == 'disappearing_reappearing':
            anomaly['anomaly_type'] = anomaly_type

            # Create the first part of the anomaly (before disappearing)
            # Make a copy of the base flight data
            anomaly_before = base_flight.copy()
            anomaly_before['anomaly_type'] = 'disappearing_reappearing_before'
            anomalies.append(anomaly_before)

            # Create the "after reappearing" version with significant position change
            # Large displacement in both latitude and longitude
            # For context: 1 degree of latitude is approximately 60 nautical miles
            lat_jump = np.random.uniform(0.5, 2.0) * np.random.choice([-1, 1])  # 30-120 NM jump
            lon_jump = np.random.uniform(0.5, 2.0) * np.random.choice([-1, 1])  # 30-120 NM jump

            # Apply the jumps
            anomaly['lat'] = float(base_flight['lat']) + lat_jump
            anomaly['lon'] = float(base_flight['lon']) + lon_jump

            # Possibly change altitude and heading too (50% chance)
            if np.random.random() > 0.5:
                # Change altitude by a random amount between -5000 and +5000 feet
                alt_change = np.random.uniform(-5000, 5000)
                anomaly['alt'] = max(0, float(base_flight['alt']) + alt_change)  # Ensure non-negative altitude

                # Change heading by a random amount between -90 and +90 degrees
                heading_change = np.random.uniform(-90, 90)
                anomaly['heading'] = (float(base_flight['heading']) + heading_change) % 360

            # Set an unusually high time difference to indicate the gap
            # This would normally be caught in data processing as a tracking gap
            anomaly['time_diff'] = np.random.uniform(60, 300)  # 1-5 minute gap

            # For rate-of-change features, set them to reflect the sudden jump
            if 'gs_change_rate' in anomaly:
                # Reset or keep the speed stable as the anomaly is about position
                anomaly['gs_change_rate'] = 0

            if 'heading_change_rate' in anomaly:
                # If heading changed, calculate the rate over the time gap
                if 'heading' in anomaly and anomaly['heading'] != base_flight['heading']:
                    heading_diff = ((anomaly['heading'] - float(base_flight['heading'])) + 180) % 360 - 180
                    anomaly['heading_change_rate'] = heading_diff / anomaly['time_diff']
                else:
                    anomaly['heading_change_rate'] = 0
...
def create_synthetic_anomalies(df, num_anomalies=1000):
    ...
    anomaly_types = [
        ...
        # Additional anomaly types (10% chance)
        {'type': 'disappearing_reappearing', 'prob': 0.05},
        {'type': 'repeated_flight_path', 'prob': 0.05}  # New anomaly type
    ]
    ...
    for _ in range(num_anomalies):
        ...
        elif anomaly_type == 'repeated_flight_path':
            # Generate a sequence of points that represent a repeated pattern
            # For this anomaly, we'll create multiple data points to show the repetition

            # Base parameters for the flight
            base_lat = float(base_flight['lat'])
            base_lon = float(base_flight['lon'])
            base_alt = float(base_flight['alt'])
            base_heading = float(base_flight.get('heading', 0))
            base_speed = float(base_flight.get('gs', 200))

            # Pattern parameters
            pattern_type = np.random.choice(['circle', 'figure_eight', 'parallel_sweeps'])
            num_points = 10  # Number of points to generate for this pattern

            # Reduced speed for repeated patterns (typically slower than cruise)
            reduced_speed = base_speed * np.random.uniform(0.3, 0.7)

            # Create multiple points to represent the pattern
            for i in range(num_points):
                pattern_point = base_flight.copy()
                pattern_point['anomaly_type'] = f'repeated_flight_path_{pattern_type}'
                pattern_point['gs'] = reduced_speed  # Slower speed during pattern flying

                if pattern_type == 'circle':
                    # Create circular pattern
                    # Use parametric equation for circle: (x,y) = (center_x + r*cos(t), center_y + r*sin(t))
                    radius = np.random.uniform(0.05, 0.15)  # ~3-9 NM radius
                    angle = (i / num_points) * 2 * np.pi  # Full circle over the points

                    # Calculate position on the circle
                    pattern_point['lat'] = base_lat + radius * np.cos(angle)
                    pattern_point['lon'] = base_lon + radius * np.sin(angle)

                    # Calculate heading (tangent to the circle)
                    pattern_point['heading'] = (np.degrees(angle) + 90) % 360

                    # Set consistent altitude
                    pattern_point['alt'] = base_alt
                    pattern_point['vertRate'] = 0  # No vertical movement in a perfect circle

                    # Calculate heading change rate (approximately constant for a circle)
                    if i > 0:
                        # Full 360° turn over the pattern duration
                        # Assuming each point is roughly 10-30 seconds apart
                        time_diff = np.random.uniform(10, 30)
                        pattern_point['time_diff'] = time_diff
                        deg_per_point = 360 / num_points
                        pattern_point['heading_change_rate'] = deg_per_point / time_diff

                elif pattern_type == 'figure_eight':
                    # Create figure-eight pattern using lemniscate equation
                    # Parametric equation: (x,y) = (a*sin(t)/(1+cos(t)^2), a*sin(t)*cos(t)/(1+cos(t)^2))
                    size = np.random.uniform(0.05, 0.15)  # Size of the figure-eight
                    t = (i / num_points) * 4 * np.pi  # Two full loops for a complete figure-eight

                    # Modified lemniscate equation for smoother figure-eight
                    pattern_point['lat'] = base_lat + size * np.sin(t)
                    pattern_point['lon'] = base_lon + size * np.sin(t) * np.cos(t)

                    # Heading changes more rapidly in a figure-eight
                    pattern_point['heading'] = (base_heading + 45 * np.sin(2*t)) % 360

                    # Slight altitude variations
                    pattern_point['alt'] = base_alt + 100 * np.sin(t)  # Minor altitude change during pattern
                    pattern_point['vertRate'] = 100 * np.cos(t)  # Corresponding vertical rate

                    # Calculate heading change rate
                    if i > 0:
                        time_diff = np.random.uniform(10, 30)
                        pattern_point['time_diff'] = time_diff
                        # Heading changes are more complex in figure-eight
                        # Approximate with a larger range
                        pattern_point['heading_change_rate'] = np.random.uniform(2, 8)

                elif pattern_type == 'parallel_sweeps':
                    # Create parallel back-and-forth sweeping pattern
                    # Alternate between two parallel lines
                    sweep_length = np.random.uniform(0.1, 0.3)  # Length of each sweep (~6-18 NM)
                    sweep_width = np.random.uniform(0.02, 0.05)  # Distance between parallel lines

                    # Calculate position along the sweep pattern
                    sweep_cycle = i % 4  # 0: start→end, 1: end, 2: end→start, 3: start
                    if sweep_cycle == 0:
                        # Moving from start to end of sweep line 1
                        progress = (i % 1) / 1.0
                        pattern_point['lat'] = base_lat
                        pattern_point['lon'] = base_lon + (progress * sweep_length) - (sweep_length / 2)
                        pattern_point['heading'] = 90  # East
                    elif sweep_cycle == 1:
                        # At end of sweep line 1, turning to sweep line 2
                        pattern_point['lat'] = base_lat + sweep_width
                        pattern_point['lon'] = base_lon + (sweep_length / 2)
                        pattern_point['heading'] = 0  # North
                    elif sweep_cycle == 2:
                        # Moving from end to start of sweep line 2
                        progress = (i % 1) / 1.0
                        pattern_point['lat'] = base_lat + sweep_width
                        pattern_point['lon'] = base_lon + ((1-progress) * sweep_length) - (sweep_length / 2)
                        pattern_point['heading'] = 270  # West
                    else:  # sweep_cycle == 3
                        # At start of sweep line 2, turning to sweep line 1
                        pattern_point['lat'] = base_lat
                        pattern_point['lon'] = base_lon - (sweep_length / 2)
                        pattern_point['heading'] = 180  # South

                    # Constant altitude during sweeps
                    pattern_point['alt'] = base_alt
                    pattern_point['vertRate'] = 0

                    # Calculate heading change rate
                    if i > 0:
                        # Heading changes sharply at corners, stable during sweeps
                        time_diff = np.random.uniform(10, 30)
                        pattern_point['time_diff'] = time_diff
                        if i % 1 < 0.1 or i % 1 > 0.9:  # At corners
                            pattern_point['heading_change_rate'] = 90 / time_diff  # 90° turns
                        else:
                            pattern_point['heading_change_rate'] = 0  # Stable heading during sweeps

                # Add gs_change_rate (should be minimal during pattern flying)
                pattern_point['gs_change_rate'] = np.random.uniform(-0.5, 0.5)  # Minimal speed changes

                anomalies.append(pattern_point)
    ...
    print("\nSynthetic Commercial Flight Anomalies Created:")
    for anomaly_type, count in anomaly_counts.items():
        print(f"  - {anomaly_type}: {count} instances ({count/len(anomaly_df)*100:.1f}%)")
